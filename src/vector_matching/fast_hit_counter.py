...
# Add caching to prevent recalculating the same vector pairs
from functools import lru_cache


# Wrap the function with LRU cache
@lru_cache(maxsize=1024)
def cached_match(vec1_tuple, vec2_tuple):
    """Cached version of smart_longest_consecutive_match using hashable tuples."""
    ...
    for filename in tqdm.tqdm(file_list, desc="Processing files", unit="file"):
        ...
        try:
            ...
            for key in data.files:
                ...
                vec_tuple = tuple(vec)
                match_len = cached_match(vector_tuple, vec_tuple)
...
def save_in_hits_average(input_dir, vector):
